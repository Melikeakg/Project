...
{
    public partial class FormDoctorAdd : Form
    {
        public FormDoctorAdd()
        {
            InitializeComponent();
            LoadTheme();
        }
        ...
        //Diyetisyenin kaydedilmesini sağlar.İçerisinde veritabanı bağlantısı var.
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    Con.Open();
                    string query = "insert into Doctor values('" + txtNameSurname.Text + "','" + txtTC.Text + "','" + comboGender.SelectedItem.ToString() + "','" + txtPhone.Text + "','" + dtpBirth.Text + "','" + richTxtAdress.Text + "','" + txtEmail.Text + "')";
                    SqlCommand cmd = new SqlCommand(query, Con);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Diyetisyen kaydedildi.", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    Con.Close();
                }
                catch (Exception ex)
                {
